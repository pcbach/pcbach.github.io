...
function processs(array,move){
	var resArray = [];
	for(var i = 0 ; i < array.length ; i++ ){
		if(array[i]==array[i+1]){resArray.push(array[i]*2);i++;score+=array[i]*2;move = true}
		else resArray.push(array[i]);
	}
	return [resArray,move];
}
function upMove(){
	...
	for(var j = 1 ; j <= 4 ; j++ ){
		...
		[processedArray,move] = processs(preProcessArray,move);
		for(var i = 0; i < 4 ; i++ ){
			id = key + (i+1) + "-" + j; 
			blockRef = document.getElementById(id);
